    ...
    try {
      ...
      if (sessionId && transports[sessionId]) {
        ...
        if (authHeader && authHeader.toLowerCase().startsWith("bearer ")) {
          ...
          // eslint-disable-next-line no-console
          console.warn(
            "MCP session initialized: No Bearer token found in Authorization header."
          );
        }
        ...
        server.tool(
          ...
          async ({ petId }: { petId: string }) => {
            try {
              ...
              throw new Error(
                `Failed to retrieve vaccination information: ${errorMessage}`
              );
            }
          }
        );
        ...
        server.tool(
          ...
          "Books a new veterinary appointment for a specific pet. Requires user authentication and explicit consent via an authorization token.",
          {
            date: z
              .string()
              .describe("Desired date for the appointment (e.g., YYYY-MM-DD)."),
            petId: z.string().describe("The unique identifier for the pet."),
            reason: z.string().describe("The reason for the vet visit."),
            time: z
              .string()
              .describe(
                "Desired time for the appointment (e.g., HH:MM AM/PM)."
              ),
          },
          async ({
            date,
        ...
        let message: string =
          "Bad Request: No valid session ID provided for existing session.";
        if (!isInitializeRequest(req.body)) {
          message =
            "Bad Request: Not an initialization request and no session ID found.";
        }
        res.status(400).json({
          error: {
            code: -32000,
...
const handleSessionRequest = async (
  ...
  try {
    const sessionId: string | undefined = expressReq.headers[
      "mcp-session-id"
    ] as string | undefined;
    if (!sessionId || !transports[sessionId]) {
    ...
    const { transport }: { transport: StreamableHTTPServerTransport } =
